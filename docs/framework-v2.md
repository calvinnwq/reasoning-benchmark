...
- **Suite selection:** allow named slices such as smoke, starter, full, and holdout. Calibrated default `starter` and `holdout` suite manifests live under `data/suites/` and are loaded via `scripts/suites.py`; `run_benchmark.py` honors them through `--suite NAME`. Instruction-ambiguity lives in an optional suite/config layer instead of the default benchmark set.
